...
def _value(game: "Game", name: str) -> int:
    if name in game.attrs:
        return int(game.attrs[name])
    return int(game.resources.get(name, 0))
...
class Game:
    ...
    def __init__(self) -> None:
        ...
        self.flags: dict[str, Any] = {"rewarded_nodes": [], "insights": []}
    ...
    def load_data(self, data: dict[str, Any]) -> None:
        if not isinstance(data, dict) or data.get("current_node", "start") not in NODES:
